-- ============================================================
-- MIGRATION PART 3B: 10 Demo Patients + Sessions + Routines + Notifications
-- ============================================================
...
SELECT id, 'fdf7cc26-59e3-413b-9e3a-43e54976a8e0'
FROM profiles
...
VALUES
  ('a1010001-0000-0000-0000-000000000001', 'fdf7cc26-59e3-413b-9e3a-43e54976a8e0', 'Rutina Tunel Carpiano', 'Ejercicios de movilidad y fortalecimiento', '[{"nombre":"Flexion de muneca","series":3,"repeticiones":15}]'::jsonb, true, 'activa', '2026-07-15', 'Dra. Demo Fisio'),
  ('a1010002-0000-0000-0000-000000000002', 'fdf7cc26-59e3-413b-9e3a-43e54976a8e0', 'Rutina Lumbalgia', 'Estabilizacion lumbar', '[{"nombre":"Pelvic tilt","series":3,"repeticiones":12}]'::jsonb, true, 'activa', '2026-07-15', 'Dra. Demo Fisio'),
  ('a1010003-0000-0000-0000-000000000003', 'fdf7cc26-59e3-413b-9e3a-43e54976a8e0', 'Rutina Hombro', 'Fortalecimiento del manguito rotador', '[{"nombre":"Pendular","series":3,"repeticiones":15}]'::jsonb, true, 'activa', '2026-07-15', 'Dra. Demo Fisio'),
  ('a1010004-0000-0000-0000-000000000004', 'fdf7cc26-59e3-413b-9e3a-43e54976a8e0', 'Rutina Rodilla', 'Fortalecimiento cuadriceps', '[{"nombre":"Sentadilla parcial","series":3,"repeticiones":12}]'::jsonb, true, 'activa', '2026-07-15', 'Dra. Demo Fisio'),
  ('a1010005-0000-0000-0000-000000000005', 'fdf7cc26-59e3-413b-9e3a-43e54976a8e0', 'Rutina Tobillo', 'Propriocepcion y estabilidad', '[{"nombre":"Circunduccion","series":3,"repeticiones":15}]'::jsonb, true, 'activa', '2026-07-15', 'Dra. Demo Fisio'),
  ('a1010006-0000-0000-0000-000000000006', 'fdf7cc26-59e3-413b-9e3a-43e54976a8e0', 'Rutina Cervical', 'Movilidad cervical', '[{"nombre":"Rotacion cervical","series":3,"repeticiones":10}]'::jsonb, true, 'activa', '2026-07-15', 'Dra. Demo Fisio'),
  ('a1010007-0000-0000-0000-000000000007', 'fdf7cc26-59e3-413b-9e3a-43e54976a8e0', 'Rutina Codo', 'Fortalecimiento epicondileos', '[{"nombre":"Extension de muneca","series":3,"repeticiones":15}]'::jsonb, true, 'activa', '2026-07-15', 'Dra. Demo Fisio'),
  ('a1010008-0000-0000-0000-000000000008', 'fdf7cc26-59e3-413b-9e3a-43e54976a8e0', 'Rutina Hernia L4-L5', 'Estabilizacion core', '[{"nombre":"Cat-camel","series":3,"repeticiones":12}]'::jsonb, true, 'activa', '2026-07-15', 'Dra. Demo Fisio'),
  ('a1010009-0000-0000-0000-000000000009', 'fdf7cc26-59e3-413b-9e3a-43e54976a8e0', 'Rutina Rotuliana', 'Fortalecimiento VMO', '[{"nombre":"Mini-squat","series":3,"repeticiones":15}]'::jsonb, true, 'activa', '2026-07-15', 'Dra. Demo Fisio'),
  ('a1010010-0000-0000-0000-000000000010', 'fdf7cc26-59e3-413b-9e3a-43e54976a8e0', 'Rutina Cadera', 'Movilidad y fortalecimiento gluteo', '[{"nombre":"Puente gluteo","series":3,"repeticiones":15}]'::jsonb, true, 'activa', '2026-07-15', 'Dra. Demo Fisio')
ON CONFLICT DO NOTHING;
...
-- Post-session reports (FK now fixed to profiles)
...
WHERE s.paciente_id IN ('a1010001-0000-0000-0000-000000000001','a1010002-0000-0000-0000-000000000002','a1010003-0000-0000-0000-000000000003')
AND s.fecha IS NOT NULL
ON CONFLICT DO NOTHING;

-- Notifications for the demo fisioterapeuta
INSERT INTO notifications (user_id, type, title, message, link, read, created_at)
VALUES
  ('fdf7cc26-59e3-413b-9e3a-43e54976a8e0', 'sesion', 'Sesion completada', 'Paciente Maria Rodriguez completo su sesion de hoy', NULL, false, NOW() - INTERVAL '2 hours'),
  ('fdf7cc26-59e3-413b-9e3a-43e54976a8e0', 'sistema', 'Baja adherencia', 'Paciente Carlos Gomez tiene baja adherencia esta semana (40%)', NULL, false, NOW() - INTERVAL '5 hours'),
  ('fdf7cc26-59e3-413b-9e3a-43e54976a8e0', 'rutina', 'Actualizacion pendiente', 'Nueva actualizacion de diagnostico pendiente para Ana Martinez', NULL, false, NOW() - INTERVAL '8 hours'),
  ('fdf7cc26-59e3-413b-9e3a-43e54976a8e0', 'sistema', 'Recordatorio de sesion', 'Recordatorio: Sesion programada con Luis Fernandez manana a las 10:00 AM', NULL, false, NOW() - INTERVAL '1 day')
ON CONFLICT DO NOTHING;